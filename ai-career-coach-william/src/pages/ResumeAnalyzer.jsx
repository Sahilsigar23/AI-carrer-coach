...
const ResumeAnalyzer = () => {
  ...
  const handleFileUpload = (event) => {
    ...
    if (file && (file.type === 'application/pdf' || file.type.includes('word') || /\.(pdf|doc|docx)$/i.test(file.name))) {
  ...
  const handleAnalyze = async () => {
    ...
    setError(null);
    try {
      const { data } = await ai.resumeAnalyze(selectedFile);
...
=== POSITIVE FEEDBACK ===
${analysisResults.positiveFeedback?.map((item, index) => `${index + 1}. ${item}`).join('\n') || 'No positive feedback available'}

=== AREAS FOR IMPROVEMENT ===
${analysisResults.areasForImprovement?.map((item, index) => `${index + 1}. ${item}`).join('\n') || 'No improvement areas identified'}

=== MISSING SKILLS ===
${analysisResults.missingSkills?.map((item, index) => `${index + 1}. ${item}`).join('\n') || 'No missing skills identified'}

=== SUGGESTIONS ===
${analysisResults.suggestions?.map((item, index) => `${index + 1}. ${item}`).join('\n') || 'No suggestions available'}

=== OVERALL SCORE ===
${analysisResults.overallScore || 'Not available'}

=== SUMMARY ===
${analysisResults.summary || 'No summary available'}

---
Report generated by AI Career Coach
    `.trim();
      ...
      <div className="p-4 md:p-8">
              ...
              <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <BiDownload 
                    ...
                    className="text-indigo-400 hover:text-gray-900 cursor-pointer" 
                    size={20} 
                  />
                </div>
                <ul className="space-y-2">
          ...
          {analysisResults && (
            <div className="flex flex-col sm:flex-row gap-4 mt-8">
              <button 
                onClick={handleDownloadReport}
                className="flex-1 bg-indigo-500 text-white py-3 px-6 rounded-full font-semibold hover:bg-indigo-600 transition-colors duration-300"
              >
              ...
              <button className="flex-1 bg-gray-100 text-gray-900 py-3 px-6 rounded-full font-semibold hover:bg-gray-200 transition-colors duration-300" onClick={handleResetAnalysis}>
                Upload New Resume
              </button>
            </div>
          )}
